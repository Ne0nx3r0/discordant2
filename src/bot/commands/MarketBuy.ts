...
export default class MarketBuy extends Command{
    ...
    async run(bag:CommandRunBag){
        ...
        bag.message.channel.sendMessage(`Purchased ${purchased.amountPurchased} ${item.title} from offer ${offerSid} for ${purchased.totalCost}GP, ${bag.message.author.username}`);

        const suffix = purchased.amountLeft == 0 ? 'all items sold, offer ended!' : purchased.amountLeft+' left';

        bag.message.client.users.get(purchased.sellerUid).sendMessage(`${bag.message.author.username} (${bag.message.author.id}) bought ${purchased.amountPurchased} ${item.title} from offer ${offerSid}, ${suffix}`);
    }
}